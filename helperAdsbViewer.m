classdef helperAdsbViewer < handle
  ...
  methods
    ...
    function stopProgressBar(obj)
      obj.GUIHandles.ProgressIndicator.String  = 'Stopped';
    ...
    function startSourceStatus(obj)
        ...
        elseif strcmp(sigSrcType, 'RTLSDRRadio') || strcmp(sigSrcType,'PlutoSDRRadio') || strcmp(sigSrcType,'USRPRadio')
    ...
    function startDataLog(obj,logfilename)
        ...
        if ~strcmp(fileName, obj.LogFileName)
          ...
          if obj.FileHandle ~= -1
            obj.LogData = true;
            % BURAYI DEĞİŞTİRDİK: Sadece arayüzdeki başlıklar yazılacak
            fprintf(obj.FileHandle, 'Aircraft ID, Flight ID, Latitude, Longitude, Altitude, Speed, Heading, Vertical Rate, Time\n');
          else
  ...
  methods (Access = private)
      ...
      if any(~strncmp(tableData(:,ADSBFieldNames.Current),'',1))
        ...
        for idx = 1:obj.MaxNumMessages
          if strncmp(tableData{idx,ADSBFieldNames.ICAO24},  '',1)
            ...
            tableData{idx,ADSBFieldNames.Time} = ...
              datestr(tableData{idx,ADSBFieldNames.Time},'HH:MM:SS');
          end
        end
    ...
    function renderGUI(obj)
      if ~ishandle(obj.ParentHandle) || ~isvalid(obj.ParentHandle)
        obj.FigureHandle = uifigure('Position', [100 100 990 516], ...
          'Visible', 'off', ...
          'HandleVisibility', 'on', ...
          'Color', [0.8 0.8 0.8], ...
          'MenuBar', 'none', ...
        ...
        obj.ParentHandle = uipanel(obj.FigureHandle, ...
      ...
      hMain = uipanel('Parent', obj.ParentHandle, ...
          'BorderType', 'none', 'AutoResizeChildren', 'off', ...
          'Units', 'Normalized', 'Position', [0 0 1 1]);
      ...
      hProgressBarIndicatorContainer = ...
        uipanel(hProgressBarContainer, 'BorderType', 'none', ...
        'AutoResizeChildren', 'off', ...
        'Units', 'Normalized', 'Position', [0 0 1 1]);
      ...
      % (1.3) Lost flag
      hLateLost = uipanel(obj.ParentHandle, 'BorderType', 'none', ...
          'AutoResizeChildren', 'off', ...
          'Units', 'Normalized', 'Position', [0 0 1 1]);
      ...
      % (1.3.5) Empty space
      hEmpty = uipanel(hLateLost, 'BorderType', 'none', ...
          'AutoResizeChildren', 'off', ...
          'Units', 'Normalized', 'Position', [0 0 1 1]);
      ...
      hUpperContainer = uipanel(obj.ParentHandle, 'BorderType', 'none', ...
          'AutoResizeChildren', 'off', ...
          'Units', 'Normalized', 'Position', [0 0 1 1]);
    ...
    function write2File(obj,msg,msgCnt)
      if msgCnt > 0
        tableData = obj.TableData; % Arayüzdeki toparlanmış tabloyu alıyoruz
        
        for p=1:msgCnt
          currMsg = msg(p);
          
          % Sadece CRC hatası olmayan geçerli paketlerde loglama yap
          if currMsg.Header.CRCError == false
              
            % Bu paketi gönderen uçağın tablodaki satır indeksini bul
            idx = find(strcmp(tableData(:,ADSBFieldNames.ICAO24), currMsg.ICAO24));
            
            if ~isempty(idx)
              idx = idx(1); % İlk eşleşen satırı al
              
              % Tablodaki hücre değerlerini çek
              icao     = tableData{idx, ADSBFieldNames.ICAO24};
              flightId = tableData{idx, ADSBFieldNames.FlightID};
              lat      = tableData{idx, ADSBFieldNames.Latitude};
              lon      = tableData{idx, ADSBFieldNames.Longitude};
              alt      = tableData{idx, ADSBFieldNames.Altitude};
              spd      = tableData{idx, ADSBFieldNames.Speed};
              hdg      = tableData{idx, ADSBFieldNames.Heading};
              vrate    = tableData{idx, ADSBFieldNames.VerticalRate};
              t        = tableData{idx, ADSBFieldNames.Time};
              
              % Boş olabilecek değerleri "N/A" veya uygun string formatına çevir
              if isempty(flightId), flightId = 'N/A'; end
              if isnumeric(lat) && isnan(lat), latStr = 'N/A'; elseif isnumeric(lat), latStr = num2str(lat, '%.4f'); else, latStr = lat; end
              if isnumeric(lon) && isnan(lon), lonStr = 'N/A'; elseif isnumeric(lon), lonStr = num2str(lon, '%.4f'); else, lonStr = lon; end
              if isnumeric(alt), altStr = num2str(alt); else, altStr = alt; end
              if isnumeric(spd), spdStr = num2str(spd); else, spdStr = spd; end
              if isempty(hdg), hdg = 'N/A'; end
              if isnumeric(vrate), vrateStr = num2str(vrate); else, vrateStr = vrate; end
              if isempty(t) || (~ischar(t) && t == 0), timeStr = 'N/A'; else, timeStr = datestr(t, 'HH:MM:SS'); end
              
              % Yeni temiz formatla dosyaya yazdır
              fprintf(obj.FileHandle, '%s, %s, %s, %s, %s, %s, %s, %s, %s\n', ...
                icao, flightId, latStr, lonStr, altStr, spdStr, hdg, vrateStr, timeStr);
            end
          end
        end
      end
